"""glob tool — find files by glob pattern (OpenCode-style).
...
def _glob_sync(
    ...
    if _has_ripgrep():
        result = _glob_ripgrep(pattern, search_path)
    else:
        result = _glob_pure(pattern, search_path)
...
def glob(
    ...
    """
    return _glob_sync(pattern, path, ctx=ctx)
