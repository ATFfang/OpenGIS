...
Safety is handled by the permission gate, workspace-scoped execution, and
script archiving. The sandbox philosophy here is explicitly
**"Claude Code-style", not "Docker-style"**.

Protocol
--------
See ``execution/subprocess_runner.py`` for the full wire format. In short:
...
class SubprocessExecutorConfig:
    ...
    # that's running us. Callers may replace this with the workspace-local
    # venv so pip installs stay per-project.
    python_executable: str = field(default_factory=lambda: sys.executable)

    # Working directory for the child. Chat-layer callers should pass the
    # workspace root; None → child inherits cwd.
    ...
    * ``cleanup() -> None``
    ...
    def send_tools(self, tools: dict[str, Any]) -> None:
        ...
        self._send({
            ...
            # Reserved for future import policy enforcement. The subprocess
            # currently ignores this because import whitelists are not enabled.
            "authorized_imports": ["*"],
        })
    ...
    def _ensure_child(self) -> None:
        ...
        self._proc = subprocess.Popen(
            [
                ...
                "-u",
                "-m",
                "opengis_backend.agent.execution.subprocess_runner",
            ],
