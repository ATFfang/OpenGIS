"""
Sub-agent delegation skills (Agent-as-Tool).
...
- **Wiring**: the parent stashes ``_skill_registry`` and ``_llm_config`` into
  ``ctx.meta`` (see ``GISCodeAgent.run``); these skills read them back to
...
def _notify_subagent(ctx: SkillContext, payload: dict) -> None:
...
def _resolve_branch_groups(requested: Any) -> Optional[list[str]]:
    """Resolve the skill-group filter for a child loop.
...
def _run_branch(
    parent_meta: dict,
    task: str,
    skill_groups: Optional[list[str]],
    max_steps: int,
    ...
    """
    # Lazy imports: avoid any import-order coupling at skill-load time and
    ...
    if registry is None or llm_config is None:
        return {
            ...
            "ok": False,
            "error": (
                "sub-agent is not wired (missing _skill_registry / _llm_config "
...
# ── Skill: single isolated sub-agent (Tier 1) ────────────────────────────
...
# ── Skill: parallel sub-agents (Tier 2 — fan-out / gather) ────────────────
...
def run_subagents(
    ...
    groups = _resolve_branch_groups(skill_groups)
