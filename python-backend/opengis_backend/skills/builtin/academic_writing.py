"""Academic writing skills — polish, translate, grammar check, abstract generation.

Group: report (attachable, not loaded by default).

Based on GPT-Academic (github.com/binary-husky/gpt_academic) prompt patterns,
adapted for the OpenGIS skill architecture. All skills are prompt-driven —
they send the text to the LLM with academic writing instructions and return
the improved result.

These skills do NOT call the LLM directly. They return structured instructions
that the agent's LLM processes as part of its normal code execution flow.
The agent calls these skills, receives the instructions, and applies them to
