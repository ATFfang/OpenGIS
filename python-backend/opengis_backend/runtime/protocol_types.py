...
Why a separate file:
    `opengis_backend.rpc.handler` owns runtime JSON-RPC dispatch. This module
    only carries shared protocol type models, so transport handling and type
...
def get_method_channel(method: str) -> Optional[MethodChannel]:
    ...
    Returns None for unknown or non-routable methods. Callers should leave
    those messages on the broadcast notification path.
    """
