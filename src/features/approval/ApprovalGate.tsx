...
export function ApprovalInline() {
  ...
  return (
    <div className="bg-bg-primary px-2.5 py-1.5 animate-slide-up">
...
function ApprovalCard({
  ...
  const canRemember = Boolean(workspacePath && request.toolName && request.kind !== 'choose' && request.kind !== 'text')
  ...
  const cardClass = embedded
    ? `w-full max-h-[min(360px,42vh)] overflow-hidden rounded-md bg-bg-secondary/70 shadow-[0_1px_8px_rgba(0,0,0,0.08)] ${
        isCodeApproval
          ? 'border border-accent-primary/55 ring-1 ring-accent-primary/25'
          : 'ring-1 ring-black/5 dark:ring-white/5'
      }`
    : `w-[min(720px,calc(100vw-32px))] max-h-[min(760px,calc(100vh-48px))] overflow-hidden rounded-lg border bg-bg-secondary shadow-2xl ${
        isCodeApproval
  ...
  const iconWrapClass = embedded
    ? 'mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded bg-bg-primary/70'
  ...
  const footerClass = embedded
    ? 'flex items-center gap-1.5 px-3 pb-2 pt-1.5'
    : 'flex items-center gap-2 border-t border-border bg-bg-primary/45 px-4 py-3'
  const secondaryButtonClass = embedded
    ? 'inline-flex h-7 items-center gap-1 rounded bg-bg-primary/65 px-2.5 text-2xs text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary'
    : 'inline-flex h-8 items-center gap-1.5 rounded border border-border px-3 text-xs text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary'
      ...
      <div className={headerClass}>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {request.toolName && (
              <span className={`${embedded ? 'bg-bg-primary/55 px-1 py-0.5 text-[10px]' : 'border border-border/80 bg-bg-primary px-1.5 py-0.5 text-2xs'} shrink-0 rounded font-mono text-text-secondary`}>
                {request.toolName}
              </span>
            )}
          </div>
        ...
        {queueLength > 0 && (
          <span className={`${embedded ? 'px-1.5 py-0.5 text-[10px]' : 'px-2 py-1 text-2xs'} rounded-full bg-bg-primary text-text-muted`}>
            队列 +{queueLength}
          </span>
        )}
      </div>
...
function CodePreview({
  ...
  return (
    <div className={`${embedded ? 'bg-bg-primary/65' : 'bg-bg-primary'} border border-accent-primary/25 overflow-hidden rounded-md`}>
      <div className={`${embedded ? 'gap-1.5 px-2.5 py-1.5' : 'gap-2 border-b border-border px-3 py-2'} flex items-center`}>
        ...
        <button
          ...
          className={`flex w-full items-center rounded-md text-left transition-colors ${
            value === option
              ...
              : embedded
                ? 'bg-bg-primary/55 text-text-secondary hover:bg-bg-hover'
...
const TextInput = forwardRef<HTMLTextAreaElement, {
  ...
  return (
    <textarea
      ...
      className={`${embedded ? 'min-h-20 border-transparent bg-bg-primary/65 px-2.5 py-1.5 text-2xs leading-snug focus:bg-bg-primary' : 'min-h-28 border-border bg-bg-primary px-3 py-2 text-xs leading-relaxed focus:border-accent-primary'} w-full resize-y rounded-md border text-text-primary outline-none transition-colors placeholder:text-text-muted`}
    />
  )
})
